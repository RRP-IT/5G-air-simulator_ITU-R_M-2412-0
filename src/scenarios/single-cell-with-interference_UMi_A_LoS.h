...
static void SingleCellWithInterferenceUMiALoS (int argc, char *argv[])
{
  ...
  //RP
// double carrierFreq = 3500; // MHz
 // double carrierFreq = 5620; // in MHz
 double carrierFreq = 2600; // MHz
  ...
  for (int i = 0; i < nbCells; i++)
    {
      ...
      Cell *c = new Cell (i, radius, 0.001, center.GetCoordinateX (), center.GetCoordinateY ());
      cells->push_back (c);
  ...
  for (int i = 0; i < nbUE; i++)
    {
	  //ue's random position
	        ...
	        double r = radius* 1000. *r1;

	        double posX = 0.95 * r * cos (angle);
	        double posY = 0.95 * r * sin (angle);

	        double th_a=theta_double(rng_mt);
	        double speedDirection = th_a*((2*M_PI)/360);
      ...
      for (int k = 0; k < nbCells; k++)
        {
          ChannelRealization* c_dl = new ChannelRealization (gNBs->at (k), ue, ChannelRealization::CHANNEL_MODEL_MICROCELL_URBAN_A_IMT_3D);
          gNBs->at (k)->GetPhy ()->GetDlChannel ()->GetPropagationLossModel ()->AddChannelRealization (c_dl);
          ChannelRealization* c_ul = new ChannelRealization (ue, gNBs->at (k), ChannelRealization::CHANNEL_MODEL_MICROCELL_URBAN_A_IMT_3D);
