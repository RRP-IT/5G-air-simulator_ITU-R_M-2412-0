  ...
  if (src->GetNodeType() == NetworkNode::TYPE_GNODEB && dst->GetNodeType () == NetworkNode::TYPE_UE)
    {
      ...
      double distance3D = src->GetMobilityModel ()->GetAbsolutePosition ()->GetDistance3D (
      						dst->GetMobilityModel ()->GetAbsolutePosition ());
      ...
      switch(channelType)
    	...
    	case ChannelRealization::CHANNEL_MODEL_MICROCELL_URBAN_A_IMT_3D:
    		/*
    		 * According to  ---  Rep. ITU-R M.2412-0 ---
    		 */
    		Hue=1.5;
    		dbp = 4*(Henb-1)*(Hue-1)*(f/300); // 4*(h'BS)*(h'UT)*(f/c)
    		isLosType=1;
    		if(isLosType)
    		{
    			if(distance < dbp)
    			{
    				pathLoss = 22.0*log10(distance3D) + 28 + 20*log10(f*0.001);
    			}
    			else
    			{
    				pathLoss = 40*log10(distance3D) + 28 + 20*log10(f*0.001)-9*log10(dbp*dbp +(Henb-Hue)*(Henb-Hue));
    			}
    		}
    		else
    		{
